"""Схема schedule.db: создаётся при первом запуске (Render и др.)."""
...
def apply_schedule_schema(conn):
    """Создаёт таблицы, если их ещё нет (безопасно вызывать перед парсингом)."""
    conn.executescript(SCHEDULE_SCHEMA_SQL)
    conn.commit()
