...
- **guard** — `/guard`, камера телефона (библиотека в `static/vendor/`), результат — всплывающее окно со студбилетом/пропуском и фото

### Проверка с телефона до push (та же Wi‑Fi сеть)

1. На ПК: `docker compose up` или `python app.py` (слушает `0.0.0.0:5000`).
2. Узнай IP ПК: `ipconfig` → IPv4, например `192.168.0.15`.
3. На телефоне в браузере: `http://192.168.0.15:5000` (не `localhost`).
4. Войди как охранник → «Проверка QR» → «Включить камеру».
5. **iPhone:** камера по `http://IP` часто не работает — нужен HTTPS или тест только ручным вводом ссылки; **Android** обычно даёт камеру по локальному IP.

После push на сервере снова: `git pull` → миграции → `docker compose up -d --build`.
